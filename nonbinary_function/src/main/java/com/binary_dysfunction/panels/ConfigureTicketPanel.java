...
public class ConfigureTicketPanel extends JPanel {
    ...
    public ConfigureTicketPanel(HomeFrame currentFrame) {
        ...
        JLabel titleLabel = new JLabel("Ticket bearbeiten");
        ...
        eventNameSelectBox.addActionListener(e -> {
            ...
            searchTicketIDBox.setModel(new DefaultComboBoxModel<>(JSONConfigurations.getTickets(eventNameSelectBox.getSelectedItem()).toArray()));
            try {
        ...
        JButton submitButton = new JButton("Übernehmen");
        submitButton.setBackground(Colors.greenButtonColor);
        submitButton.setForeground(Color.WHITE);
        submitButton.setFont(new Font("Arial", Font.PLAIN, 14));
        submitButton.addActionListener((ActionEvent e) -> {
            try {
                JSONConfigurations.changeTicketValue(currentEventName, searchTicketIDBox.getSelectedItem(), "owner", ownerTextField.getText());
                if (currentTicket.available && !ownerTextField.getText().equals("")) {
                    JSONConfigurations.changeTicketValue(currentEventName, searchTicketIDBox.getSelectedItem(), "available", false);
                    availableCheckBox.setSelected(false);
                } else if (ownerTextField.getText().equals("")) {
                    JSONConfigurations.changeTicketValue(currentEventName, searchTicketIDBox.getSelectedItem(), "available", true);
                    availableCheckBox.setSelected(true);
                }
                Toast.show(null, "Ticket", "Ticket updated", 3000, Toast.Position.BOTTOM_RIGHT, false);
            } catch (IOException ex) {}
        });
