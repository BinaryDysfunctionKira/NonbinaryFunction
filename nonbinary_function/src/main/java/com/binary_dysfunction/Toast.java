...
public class Toast {
    ...
    public static void show(Window owner, String iconPath, String title, String message, int durationMs, Position position, boolean playSound) {
        SwingUtilities.invokeLater(() -> {
            JWindow window = new JWindow(owner);
            window.setAlwaysOnTop(true);
            ...
            panel.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(Colors.backgroundColorLighter),
                BorderFactory.createEmptyBorder(12, 20, 18, 20)
            ));
            panel.setPreferredSize(new Dimension(230, 80));
            ...
            JLabel iconLabel = new JLabel(Component.scaleImage(iconPath, 50));
            panel.add(iconLabel);
